...
const getReadingTime = (post: BlogPostSummary, locale: Locale) => {
...
const BlogCard = ({ post, locale, readArticle }: { post: BlogPostSummary; locale: Locale; readArticle: string }) => {
  ...
  return (
    <article className="group overflow-hidden rounded-[28px] bg-card shadow-[0_0_0_1px_rgba(0,0,0,0.06),0_12px_30px_-22px_rgba(0,0,0,0.35)] transition-[box-shadow,transform] duration-300 hover:-translate-y-1 hover:shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_18px_42px_-22px_rgba(0,0,0,0.45)]">
      <Link to={localizePath(`/blog/${post.slug}`, locale)} className="block">
        ...
        <div className="p-6 sm:p-7">
          <div className="mb-5 flex flex-wrap items-center gap-2 text-xs font-semibold text-primary">
            {post.category ? <span className="rounded-full bg-primary/10 px-3 py-1">{post.category}</span> : null}
	            {post.publishedAt ? <time className="text-muted-foreground" dateTime={post.publishedAt}>{formatDate(post.publishedAt, locale)}</time> : null}
            <span className="inline-flex items-center gap-1 text-muted-foreground">
              <Clock3 className="h-3.5 w-3.5" />
	              {getReadingTime(post, locale)}
            </span>
          </div>
          ...
          <div className="mt-7 inline-flex min-h-11 items-center text-sm font-semibold text-foreground transition-colors group-hover:text-primary">
            {readArticle} <ArrowRight className="ml-2 h-4 w-4" />
          </div>
        </div>
      </Link>
    </article>
...
const BlogIndex = () => {
  ...
  usePageSeo({
    ...
    locale,
    alternatePaths: alternatePathsFor("/blog"),
  });
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-7xl">
          ...
          {!isSanityConfigured ? (
            <div className="mx-auto max-w-2xl rounded-[28px] bg-muted/60 p-8 text-center shadow-[0_0_0_1px_rgba(0,0,0,0.06)]">
	              <h2 className="font-display text-2xl font-bold text-foreground">{t.blog.notConfiguredTitle}</h2>
	              <p className="mt-3 text-muted-foreground">
	                {t.blog.notConfiguredText}
	              </p>
            </div>
          ) : isLoading ? (
            ...
            <div className="mx-auto max-w-2xl rounded-[28px] bg-muted/60 p-8 text-center shadow-[0_0_0_1px_rgba(0,0,0,0.06)]">
	              <h2 className="font-display text-2xl font-bold text-foreground">{t.blog.loadErrorTitle}</h2>
	              <p className="mt-3 text-muted-foreground">{t.blog.loadErrorText}</p>
            </div>
          ) : posts.length === 0 ? (
            <div className="mx-auto max-w-2xl rounded-[28px] bg-muted/60 p-8 text-center shadow-[0_0_0_1px_rgba(0,0,0,0.06)]">
	              <h2 className="font-display text-2xl font-bold text-foreground">{t.blog.emptyTitle}</h2>
	              <p className="mt-3 text-muted-foreground">{t.blog.emptyText}</p>
            </div>
          ) : (
            <div className="space-y-14">
              <div className="grid gap-6 lg:grid-cols-2">
	                {posts.map((post) => <BlogCard key={post._id} post={post} locale={locale} readArticle={t.blog.readArticle} />)}
              </div>
              ...
              <section className="rounded-[28px] bg-foreground px-6 py-10 text-center text-background shadow-[0_18px_60px_-32px_rgba(0,0,0,0.55)] sm:px-10">
	                <p className="text-sm font-semibold text-background/70">{t.blog.ctaSmall}</p>
                <h2 className="mx-auto mt-3 max-w-3xl text-balance font-display text-3xl font-bold sm:text-5xl">
	                  {t.blog.ctaTitle}
	                </h2>
	                <a href={localizePath("/#cta", locale)} className="mt-7 inline-flex min-h-11 items-center rounded-full bg-background px-5 py-3 text-sm font-bold text-foreground transition-transform hover:scale-[0.98] active:scale-[0.96]">
	                  {t.blog.ctaButton} <ArrowRight className="ml-2 h-4 w-4" />
                </a>
              </section>
            </div>
          )}
      ...
      <footer className="border-t border-border px-6 py-8 text-center text-sm text-muted-foreground">
	        <Link to={localizePath("/", locale)} className="font-semibold text-foreground transition-colors hover:text-primary">Charlando</Link>
